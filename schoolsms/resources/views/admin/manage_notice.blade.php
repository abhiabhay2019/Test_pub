...
<!-- page content -->
    ...
    <div class="row">
        ...
        <table class="table table-bordered table-striped jambo_table bulk_action">
            ...
            <tbody>
                ...
                @if($notice->STATUS =='Deactive')
                <tr style="background-color: #EE756F;">
                    
                    @else
                <tr style="background-color: #A6FBCD;">    
                    @endif
                    <td><input type="radio" name="notice_id" class="notice_id" id="notice_id" value="{{$notice->ID}}" onclick="update_notice(this.value)"></td>
                    <td>{{ ++$key }}</td>
                    <td>{{ $notice->DATE }}</td>
                    <td>{{ $notice->TITLE }}</td>
                    <td>{{ $notice->STATUS }}</td>
                    <td><a href="manage-notices?noticeid={{ $notice->ID }}" class="btn classic-btn btn-sm">Edit</a></td>
  ...
  <h3 class="text-center">{{Session()->get('msg')}}</h3>
    ...
    @endphp
    <div class="x_panel">
            <form action="{{url('/admin/addnotice')}}" method="post" enctype="multipart/form-data">
                @csrf
...
function send_notice(){
   ...
   $.ajax({
        url:'sendNotice',
        data:'sendNotice=sendNotice&notice_id='+nid,
        dataType:'Json',
